...
export async function GET(
  ...
  // Strict Admin / HR authorization required to access candidate resumes
  const user = await requireUser(["admin", "employee"]);
  if (!user) {
    return new NextResponse("Unauthorized. Admin authentication required to access applicant documents.", {
      status: 401,
