...
export default function CustomerDashboard() {
  ...
  if (!user && !loading) {
    ...
    return (
      <main className="portal" style={{ maxWidth: 460, margin: "60px auto", padding: "0 20px" }}>
        <Link href="/" className="back">← Queens Care</Link>
        <p className="eyebrow">Your care account</p>
        <h1 style={{ marginBottom: 20 }}>{forgotMode ? "Reset password" : authTab === "login" ? "Sign in" : "Create account"}</h1>

        {/* Google OAuth Configuration Notice if not configured */}
        {queryError === "google_not_configured" && (
          <div style={{ padding: 16, background: "#fff9e6", border: "1px solid #ffe58f", marginBottom: 20, fontSize: 12, lineHeight: 1.5, color: "#874d00" }}>
            <b style={{ display: "block", marginBottom: 6, fontSize: 13 }}>⚙️ Google OAuth Setup Required</b>
            <p style={{ margin: "0 0 8px" }}>To enable real Google authentication, add these keys to your <code>.env.local</code> file:</p>
            <pre style={{ background: "#fff", padding: 8, border: "1px solid #ffd591", fontSize: 11, overflowX: "auto", margin: "0 0 8px" }}>
{`GOOGLE_CLIENT_ID=your_client_id.apps.googleusercontent.com
GOOGLE_CLIENT_SECRET=your_client_secret
NEXT_PUBLIC_SITE_URL=http://localhost:3000
AUTH_SECRET=your_secret_key`}
            </pre>
            <p style={{ margin: 0, fontSize: 11 }}>Authorized redirect URI in Google Cloud Console:<br /><code>http://localhost:3000/api/auth/google/callback</code></p>
          </div>
        )}
        ...
        {!forgotMode && (
          <div style={{ marginBottom: 20 }}>
            ...
            >
              ...
              <span>{authTab === "login" ? "Sign in with Google" : "Sign up with Google"}</span>
            </a>

            <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "16px 0" }}>
              <div style={{ flex: 1, height: 1, background: "var(--line)" }} />
              <span style={{ fontSize: 11, color: "var(--muted)", textTransform: "uppercase", letterSpacing: ".06em" }}>or with email</span>
              <div style={{ flex: 1, height: 1, background: "var(--line)" }} />
            </div>
          </div>
        )}

        {authMessage && <p style={{ padding: "10px 14px", background: authMessage.includes("error") || authMessage.includes("Error") || authMessage.includes("Failed") ? "#fce4ec" : "#e9f7e9", color: authMessage.includes("error") || authMessage.includes("Error") ? "#b34141" : "#2e7d32", fontSize: 12, marginBottom: 16 }}>{authMessage}</p>}
        {forgotMode ? (
