...
export default function QueensCareExperience() {
  ...
  return <div className="site-shell">
    ...
    <main><CareScene />
      ...
      <section className="journal section" id="journal"><div className="section-head"><div><p className="eyebrow">{getHpContent("newsletter").eyebrow || "The care journal"}</p><h2>{getHpContent("newsletter").heading || "Intelligence for a<br/><em>life well lived.</em>"}</h2></div><Link href="/blog" className="text-link">Read the journal <span>→</span></Link></div><div className="journal-grid"><article><img src="https://images.unsplash.com/photo-1559757148-5c350d0d3c56?auto=format&fit=crop&w=900&q=80" alt="Fresh morning fruit"/><p>WELLNESS NOTES · 6 MIN READ</p><h3>Why your afternoon slump is not a personality flaw</h3><Link href="/blog/afternoon-slump-not-a-personality-flaw">Read story →</Link></article><article><img src="https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?auto=format&fit=crop&w=900&q=80" alt="Health consultation"/><p>EXPERT SERIES · 4 MIN READ</p><h3>The truth about what &ldquo;clinically studied&rdquo; really means</h3><Link href="/blog/what-clinically-studied-really-means">Read story →</Link></article><article className="journal-cta"><span>THE<br/>CARE<br/>LETTER</span><h3>{getHpContent("newsletter").heading || "A smarter kind of inbox."}</h3><p>{getHpContent("newsletter").subtitle || "Thoughtful dispatches on science, care, and living well."}</p><form onSubmit={subscribe}><input type="email" required aria-label="Your email address" placeholder="Your email address"/><button aria-label="Subscribe">→</button></form></article></div></section>
      <section className="consult"><div><p className="eyebrow">{getHpContent("consult").eyebrow || "Care, with a human on the other end"}</p><h2>{getHpContent("consult").heading || "Questions deserve<br/>thoughtful answers."}</h2><p>{getHpContent("consult").description || "Our care team is here to help you make confident choices — no pressure, no jargon."}</p><Link href={getHpContent("consult").ctaLink || "/contact"} className="button">{getHpContent("consult").ctaText || "Talk to our care team"} <span>→</span></Link></div><div className="consult-image"><img src={getHpContent("consult").imageUrl || "https://images.unsplash.com/photo-1559839734-2b71ea197ec2?auto=format&fit=crop&w=1000&q=85"} alt="Doctor speaking with a patient"/></div></section>
    </main>
