  ...
  return <div className="site-shell">
    ...
    <main><CareScene />
      ...
      <section className="collection section" id="collection"><div className="section-head"><div><p className="eyebrow">The care edit</p><h2>Considered essentials<br/>for your <em>whole self.</em></h2></div><a href="#collection" className="text-link">Shop all care <span>→</span></a></div><div className="category-row">{categories.map((cat, index) => <button key={cat} onClick={() => setSearch(cat)}><span>0{index + 1}</span>{cat}<b>→</b></button>)}</div>{loading ? (
        ...
        <>
          <div className="product-grid">{shown.map((product) => <article className="product" key={product.id}><div className="product-image"><Image src={product.image} alt={product.name} fill sizes="(max-width: 650px) 50vw, 25vw"/><span className="pill">{product.tag}</span><button className="heart" onClick={() => toggleWish(product.name)} aria-label={`Save ${product.name}`}>{wish.includes(product.name) ? "♥" : "♡"}</button><button className="quick-add" onClick={() => add(product)}>Add to bag</button></div><p>{product.category}</p><h3>{product.name}</h3><small>{product.note}</small><div className="price"><b>₹{product.price.toLocaleString("en-IN")}</b><span>★★★★★</span></div></article>)}</div>{shown.length === 0 && products.length > 0 && <p className="empty">No results yet. Try &ldquo;wellness&rdquo; or choose a care category above.</p>}
        </>
      )}</section>
